...
export default function ActivitiesScreen() {
  ...
  const [claimForm, setClaimForm] = useState({ campaign: '', notes: '', evidenceurl: '', filename: '' });
  ...
  async function submitClaim() {
    ...
    try {
      ...
      setClaimFor(null);
      setClaimForm({ campaign: '', notes: '', evidenceurl: '', filename: '' });
      await reload();
    } catch (e) {
            ...
            <div className="grid grid-cards">
              {shownActivities.map((a) => (
                <div className="card entity-card" key={a.abs_activityid}>
                  ...
                  {currentChampion ? (
                    <button className="btn btn-primary btn-sm btn-block" onClick={() => { setClaimFor(a); setClaimForm({ campaign: '', notes: '', evidenceurl: settings?.crd49_sharepointurl ?? '', filename: '' }); }}>
                      Claim Activity
                    </button>
                  ) : (
      ...
      {claimFor && (
        ...
        >
          ...
          <div className="field-row mt-16">
            <Field label="Evidence URL"><input className="input" value={claimForm.evidenceurl} onChange={(e) => setClaimForm({ ...claimForm, evidenceurl: e.target.value })} placeholder="https://…" /></Field>
            <Field label="File name"><input className="input" value={claimForm.filename} onChange={(e) => setClaimForm({ ...claimForm, filename: e.target.value })} placeholder="certificate.pdf" /></Field>
          </div>
        </Modal>
      )}
    </>
